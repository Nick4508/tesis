...
El presente capítulo detalla el diseño, la estructura de la metodología y la implementación del sistema propuesto para la sonificación de datos de \textit{4D Flow MRI}. Para garantizar la escalabilidad y coherencia física de los resultados, el desarrollo de la solución no se abordó como un proceso único, sino que fue dividido en 3 etapas, las cuales dictan la organización de este capítulo.

\begin{enumerate}
    \item \textbf{Exploración espacial y síntesis acústica:} Esta etapa está enfocada en el preprocesamiento de los datos anatómicos y su traducción acústica. Se detallan la integración del \textit{centerline}, su interpolación espacial, el cálculo iterativo de los planos de corte ortogonales y la formulación trigonométrica para extraer las proyecciones de velocidades. Además, se describe el motor de la síntesis digital de señales encargado de mapear la cinemática del fluido a parámetros sonoros. Todo el procesamiento se consolida dentro de una interfaz gráfica interactiva, la cual permite la inspección y exploración de los diferentes planos y sus flujos. \label{item:etapa1}
    
    \item \textbf{Procesamiento masivo y extracción de características:} Esta segunda etapa se centra en la automatización del flujo de trabajo y la consolidación de la información. Se detalla la implementación de rutinas de procesamiento por lotes (\textit{batch processing}) para extraer y almacenar de forma masiva las proyecciones de velocidad de los diferentes casos de estudio que se ingresan al sistema. La etapa concluye con la construcción de un conjunto de datos estructurado (\textit{dataset}) mediante la extracción automatizada de descriptores correspondientes tanto a la cinemática del fluido como a la síntesis acústica. \label{item:etapa2}

    \item \textbf{Análisis cuantitativo y exploración interactiva:} Constituye la etapa final, destinada a corroborar la eficacia de la herramienta de sonificación. Por un lado, se establece un diseño de agrupamiento estadístico sobre el conjunto de características estructurado para analizar objetivamente la correlación de las variables. Por otro lado, se detalla la arquitectura de un tablero interactivo (\textit{dashboard}) para la exploración perceptiva, permitiendo la comparación simultánea de escenarios hemodinámicos con el fin de comprobar empíricamente si las alteraciones del flujo se traducen en firmas acústicas predecibles y distinguibles. \label{item:etapa3}
\end{enumerate}

\subsection{Exploración espacial y síntesis acústica}

La primera etapa del sistema se centra en el acondicionamiento de los datos y la configuración del motor de extracción espacial. Todo el desarrollo de esta arquitectura fue implementado en el entorno de \textit{software} MATLAB. El objetivo de esta fase es aislar secciones transversales del vaso sanguíneo para poder evaluar el comportamiento hemodinámico de forma iterativa y secuencial a lo largo de toda la estructura anatómica.

\subsubsection{Interpolación del \textit{centerline} y cálculo de planos ortogonales}
...
Computacionalmente, este vector director se estima evaluando la derivada posicional mediante una aproximación de diferencias finitas con los puntos adyacentes de la curva interpolada, esto se puede ver en la ecuación (\ref{eq:diferencias_finitas}).

\begin{equation}
    \vec{N}_i = P_{i+1} - P_{i-1}
    \label{eq:diferencias_finitas}
\end{equation}
...
Con los parámetros espaciales definidos, el sistema ejecuta la sección de las matrices volumétricas correspondientes a las tres componentes espaciales de la velocidad capturadas por el resonador ($V_x$, $V_y$, $V_z$, muchas veces representadas por $V_{RL}$, $V_{AP}$ y $V_{FH}$). Este proceso se implementa mediante la extracción de un corte oblicuo (\textit{oblique slice}) a través de los volúmenes tridimensionales. La matriz bidimensional resultante contiene, en cada uno de sus píxeles, un vector de velocidad tridimensional $\vec{V} = (v_x, v_y, v_z)$ que describe el movimiento del fluido en esa región transversal específica.

Sin embargo, al realizar un corte oblicuo sobre la segmentación anatómica, la representación resultante no siempre es una región única. Con frecuencia presenta múltiples regiones aisladas o \textit{islas}. Estas discrepancias topológicas pueden ser producto de ruido inherente en la imagen, ramificaciones vasculares secundarias o vasos sanguíneos adyacentes que cruzan el mismo plano de corte.
...
En primera instancia, el algoritmo calcula la proyección vectorial de la velocidad tridimensional sobre el vector normal unitario del plano. Matemáticamente, esto se puede ver en la ecuación (\ref{eq:proyeccion_vectorial}), donde se extrae el vector proyectado $\vec{V}_{proj}$ y se calcula su magnitud absoluta o norma euclidiana:

\begin{equation}
    ||\vec{V}_{proj}|| = ||(\vec{V} \cdot \hat{n}_i) \hat{n}_i|| = \sqrt{V_{px}^2 + V_{py}^2 + V_{pz}^2}
    \label{eq:proyeccion_vectorial}
\end{equation}
...
Previo a la sumatoria, el sistema implementa una espacialización dinámica basada en la dirección del flujo. Se utiliza una función tangente hiperbólica ($\tanh$) sobre las velocidades proyectadas para calcular coeficientes de ponderación estéreo según las ecuaciones (\ref{eq:paneo_derecho}) y (\ref{eq:paneo_izquierdo}). Este enfoque permite enrutar suavemente los flujos anterógrados (velocidades positivas) hacia el canal derecho y los flujos retrógrados (velocidades negativas) hacia el canal izquierdo:

\begin{equation}
    W_{der} = 0.5 \cdot \left(1 + \tanh(v(t))\right) 
    \label{eq:paneo_derecho}
\end{equation}

\begin{equation}
    W_{izq} = 1 - W_{der} 
    \label{eq:paneo_izquierdo}    
\end{equation}

La consolidación del sonido se logra multiplicando cada onda acústica individual por su respectivo peso espacial y aplicando la sumatoria a todas las componentes. Esta operación materializa la síntesis aditiva mediante las ecuaciones (\ref{eq:sintesis_derecha}) y (\ref{eq:sintesis_izquierda}).

\begin{equation}
    S_{der}(t) = \sum_{i=1}^{N} \cos(\Phi_i(t)) \cdot W_{der, i} 
    \label{eq:sintesis_derecha}
\end{equation}
\begin{equation}
    S_{izq}(t) = \sum_{i=1}^{N} \cos(\Phi_i(t)) \cdot W_{izq, i} 
    \label{eq:sintesis_izquierda}
\end{equation}
...
Finalmente, aplicando los principios de la síntesis sustractiva, las señales estéreo transitan por una etapa de filtrado digital discreto. Se implementan filtros tipo Butterworth de fase cero (\textit{filtfilt}) \cite{Gustafsson1996} para esculpir la señal acústica sin introducir distorsiones de retardo temporal:
...
La segunda etapa del desarrollo consiste en transicionar de una herramienta de exploración visual a un motor de medición y sonificación automatizado. Para evaluar el impacto de diferentes morfologías y condiciones de flujo, es necesario extraer descriptores matemáticos discretos a partir de las matrices generadas en la primera fase. Esta etapa detalla la optimización del procesamiento por lotes y la parametrización de las señales cinemáticas y acústicas.

\subsubsection{Procesamiento secuencial y gestión de memoria}

Dada la alta dimensionalidad de los volúmenes de \textit{4D Flow MRI}, la evaluación iterativa de las diferentes secciones transversales resulta inviable de realizar manualmente. Para abordar este desafío y evitar la rápida saturación de la memoria volátil (\textit{RAM}) al operar con matrices de gran tamaño, el flujo de trabajo masivo se estructuró mediante dos procesos independientes y asíncronos.

El primer proceso actúa como un orquestador de extracción geométrica. Su función es iterar secuencialmente sobre la topología del vaso, aislando la proyección de las velocidades de cada plano transversal. Estas se exportan inmediatamente al almacenamiento local en formato de arreglos binarios y liberan la memoria volátil. Este diseño transforma el modelo tridimensional pesado en un repositorio de datos fragmentado y ligero.

Una vez finalizada la etapa de extracción, entra en operación el segundo proceso: el motor de síntesis y generación de características. Este módulo independiente lee de forma iterativa los arreglos binarios previamente almacenados y los procesa a través del algoritmo de emulación Doppler. Al aislar la carga de trabajo y operar exclusivamente sobre datos bidimensionales preprocesados, este segundo motor logra generar los espectrogramas y sintetizar las respuestas acústicas de manera ininterrumpida, habilitando así la cuantificación final de los datos.

\subsubsection{Parametrización cinemática y acústica (\textit{Feature Extraction})}

Para llevar a cabo esta cuantificación, el sistema ejecuta rutinas de parametrización encargadas de traducir los campos vectoriales y las ondas de sonido recién sintetizadas en métricas escalares concretas. Para garantizar una comparación coherente a lo largo de toda la estructura, el sistema implementa una sincronización temporal dinámica basada en el caudal volumétrico.

Computacionalmente, la integración espacial del campo de velocidades se ejecuta mediante la sumatoria discreta de las magnitudes absolutas de todos los vóxeles válidos para cada fase temporal $t$, esto se puede ver en la ecuación (\ref{eq:caudal}).
...
\textbf{Métricas de la señal digital:} Aprovechando el motor de síntesis, las proyecciones espaciales se transforman en señales de audio. Para estandarizar el análisis, la señal es truncada a una ventana de observación correspondiente a exactamente un ciclo cardíaco. Sobre esta onda modulada se extraen diferentes descriptores de la señal de audio \cite{Davis1980,Tzanetakis2002}:
\begin{itemize}
    \item \textbf{Dominio del tiempo:} Se calcula la energía de la señal (\textit{RMS}) y la tasa de cruces por cero (\textit{Zero-Crossing Rate, ZCR}). Estas variables traducen la presión sonora aparente y la densidad de fluctuaciones de la señal acústica.
    
    \item \textbf{Dominio de la frecuencia:} Mediante la Transformada Rápida de Fourier (FFT), se proyecta la señal sonora a su espectro de frecuencias para derivar su centroide espectral y su dispersión espectral (\textit{Spectral Spread}). Estas variables modelan matemáticamente el ensanchamiento espectral (\textit{Spectral Broadening}).
    
    \item \textbf{Coeficientes cepstrales en las frecuencias de Mel (MFCC 1 al 3):} Descriptores fundamentales en el reconocimiento de patrones sonoros. Al aplicar un banco de filtros espaciados según la escala Mel, se modela la resolución no lineal de la audición humana. La extracción de estos coeficientes permite cuantificar el ``timbre'' del flujo.
\end{itemize}

La extracción sistemática de estas métricas genera una matriz de datos que unifica las características cinemáticas del flujo con su firma acústica. Esta matriz representa un conjunto de datos estructurados o \textit{dataset}. El detalle de las variables consolidadas en este conjunto, agrupadas según su dominio de origen, se expone en la Tabla \ref{tab:resumen_metricas}.

\begin{table}[htbp]
\centering
\caption{Resumen de las métricas extraídas y sus respectivos dominios.}
\label{tab:resumen_metricas}
\begin{tabular}{ll}
\toprule
\textbf{Variable / Descriptor} & \textbf{Dominio} \\ 
\midrule
Velocidad máxima (\texttt{RAW\_Vmax}) & Cinemático \\
Velocidad media (\texttt{RAW\_Vmedia}) & Cinemático \\
Varianza espacial (\texttt{RAW\_VarianzaEspacial}) & Cinemático \\
Flujo de área (\texttt{RAW\_FlujoArea}) & Cinemático \\
Asimetría espacial (\texttt{RAW\_SkewnessEspacial}) & Cinemático \\
Índice de pulsatilidad (\texttt{RAW\_IndicePulsatilidad}) & Cinemático \\ 
Energía de la señal (\texttt{WAV\_EnergiaRMS}) & Acústica \\
Amplitud pico (\texttt{WAV\_Pico}) & Acústica \\
Tasa de cruces por cero (\texttt{WAV\_ZCR}) & Acústica \\
Centroide espectral (\texttt{WAV\_CentroideFreq}) & Acústica \\
Dispersión espectral (\texttt{WAV\_SpreadFreq}) & Acústica \\
Coeficientes de Mel (\texttt{WAV\_MFCC1} a \texttt{MFCC3}) & Acústica \\ 
\bottomrule
\end{tabular}
\end{table}

\subsection{Análisis cuantitativo y exploración interactiva}

La etapa final consiste en establecer un esquema de evaluación sobre el espacio de características multimodal. Con este fin, se desarrolla una herramienta de análisis comparativo, para así contar con un mecanismo objetivo para comprobar si la traducción cinemática-acústica es efectiva, y evidenciar si las alteraciones del flujo sanguíneo generan, bajo los criterios de sonificación establecidos, firmas sonoras predecibles y distinguibles.

\subsubsection{Agrupamiento estadístico y análisis de importancia}

Para evaluar matemáticamente la capacidad de discriminación del \textit{dataset} construido, se implementa una etapa de aprendizaje automático no supervisado. El análisis se aborda mediante la ejecución del algoritmo de agrupamiento $K$-Means \cite{Jain2010}, configurado para particionar los datos en $k$ clústeres. Con el fin de aislar el peso predictivo de cada dominio, el agrupamiento se itera sobre tres subconjuntos independientes: un espacio estrictamente cinemático, un espacio puramente acústico y el espacio multimodal fusionado

La validación de los clústeres descubiertos se estructura en dos niveles. A nivel interno, se cuantifica la cohesión y separación de los grupos utilizando el coeficiente de \textit{Silhouette} \cite{Rousseeuw1987} y el índice de Calinski-Harabasz \cite{Calinski1974}. A nivel externo, se generan tablas de contingencia (\textit{cross-tables}) que cruzan las asignaciones del algoritmo con los metadatos inherentes a cada registro, es decir, el identificador del modelo físico y la condición de flujo simulada. Este cruce permite evaluar si las agrupaciones descubiertas logran segmentar de forma natural los diferentes escenarios clínicos abordados.

Adicionalmente, para comprender la contribución de cada variable en la formación de estos patrones, se aplica un Análisis de Componentes Principales (PCA) \cite{Jolliffe2016}. La extracción de los pesos absolutos del primer componente principal ($PC_1$) permite identificar cuáles descriptores actúan como los marcadores de varianza más fuertes dentro del modelo no supervisado.

\subsubsection{Exploración interactiva}

Con el fin de comparar diferentes instancias de sonificación y síntesis espectral, se genera un \textit{dashboard} interactivo. Esta interfaz se vincula directamente con las etapas anteriores. De la primera etapa hereda la reconstrucción geómetrica del vaso y la discretización espacial mediante la interpolación del \textit{centerline}. De la segunda etapa, se reutilizan las matrices guardadas en formato binario y el algoritmo de emulación Doppler. 

El \textit{dashboard} (ver Figura \ref{fig:dashboard}) utiliza estos elementos para generar pruebas comparativas simultáneas (\textit{A/B testing}) entre diferentes escenarios anatómicos o estados de flujo. Para articular analíticamente estas comparaciones, el entorno sincroniza tres dimensiones operativas principales:

\begin{itemize}
    \item \textbf{Navegación espacial:} A partir de la geometría heredada, el sistema renderiza la isosuperficie del vaso y permite al usuario desplazar un marcador a lo largo de los planos de estudio. Esto facilita la selección de la sección transversal que se desea evaluar.
    
    \item \textbf{Procesamiento y audición bajo demanda:} Al ubicar el marcador en un plano específico, la herramienta lee la matriz guardada correspondiente y ejecuta el motor de emulación. Esto permite sintetizar y reproducir de forma casi inmediata la sonificación asociada a ese plano de estudio.
    
    \item \textbf{Representación visual simultánea:} Presenta en paneles paralelos los espectrogramas y las formas de onda temporales correspondientes a cada caso. Para asegurar una comparación objetiva, el entorno unifica y ajusta dinámicamente las escalas de los ejes, garantizando que las diferencias visuales observadas correspondan a variaciones hemodinámicas.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/propuesta/dashboard.jpeg}
    \caption{Interfaz gráfica del \textit{dashboard} interactivo. El panel central permite la navegación espacial por el \textit{centerline} sobre la geometría del vaso. Los paneles laterales despliegan de forma comparativa los espectrogramas (arriba) y las formas de onda sintetizadas (abajo) para dos escenarios de flujo, integrando comandos para la evaluación acústica. Fuente: Elaboración propia.}
    \label{fig:dashboard}
\end{figure}

La relevancia de este entorno comparativo radica en su función como plataforma de evaluación perceptual. Al sincronizar la geometría del vaso con las señales sintetizadas, el sistema facilita un análisis directo de los datos procesados. Este acoplamiento es el que permite contrastar los diferentes estados de flujo y confirmar si la parametrización matemática de la hemodinámica se traduce exitosamente en marcadores sonoros reconocibles para un evaluador.
